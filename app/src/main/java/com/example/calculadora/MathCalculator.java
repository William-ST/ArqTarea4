...
public class MathCalculator implements Calculator {
    ...
    private String addOperatorBeforeParenthesis(String from, String to) {
        try {
            String previousCharacter = String.valueOf(from.charAt(
                    getParenthesisStartIndex(from) - 1));
            if (!isAnOperator(previousCharacter) &&
                    !previousCharacter.equals(PARENTHESIS_START)) return MULTIPLICATION.concat(to);
    ...
    @VisibleForTesting
    boolean isAnOperator(String symbol) {
    ...
    private boolean isABinaryOperator(String symbol) {
        return symbol.equals(ADDITION) ||
                symbol.equals(SUBTRACTION) || symbol.equals(MULTIPLICATION) || symbol.equals(DIVISION) || symbol.equals(EXPONENTIATION);
    }

    private boolean isAnUnaryOperator(String symbol) {
        return symbol.equals(SQUARE_ROOT) || symbol.equals(FACTORIAL);
    }
    ...
    private double calculateBinaryOperation(double accumulated, double value, String operator) {
        ...
        switch (operator) {
            case ADDITION:
                ...
                break;
            case SUBTRACTION:
                result = operation.substraction(accumulated, value);
                break;
            case MULTIPLICATION:
